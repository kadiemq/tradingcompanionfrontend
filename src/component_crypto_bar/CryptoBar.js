...
const CryptoBar = () => {
    ...
    function set_prices() {
        const btcwss = new WebSocket(`wss://stream.binance.com:9443/ws/btcusdt@kline_1m`)
        btcwss.onmessage = (e) => {
        ...
        const adawss = new WebSocket(`wss://stream.binance.com:9443/ws/adausdt@kline_1m`)
        adawss.onmessage = (e) => {
            const msg = JSON.parse(e.data);
            let close = msg.k.c;
            close = Number(close).toFixed(2);
        ...
        const dogewss = new WebSocket(`wss://stream.binance.com:9443/ws/dogeusdt@kline_1m`)
        dogewss.onmessage = (e) => {
            const msg = JSON.parse(e.data);
            let close = msg.k.c;
            close = Number(close).toFixed(2);
        ...
        const ethwss = new WebSocket(`wss://stream.binance.com:9443/ws/ethusdt@kline_1m`)
        ethwss.onmessage = (e) => {
            const msg = JSON.parse(e.data);
            let close = msg.k.c;
            close = Number(close).toFixed(2);
        ...
        const bnbwss = new WebSocket(`wss://stream.binance.com:9443/ws/bnbusdt@kline_1m`)
        bnbwss.onmessage = (e) => {
            const msg = JSON.parse(e.data);
            let close = msg.k.c;
            close = Number(close).toFixed(2);
